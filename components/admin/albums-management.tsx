...
import { Plus, Search, ChevronRight, Music, CheckCircle, AlertCircle, Clock } from 'lucide-react';
...
export function AlbumsManagement() {
  ...
  let filteredAlbums = albumsList.filter(album =>
  ...
  return (
    <div className="space-y-6">
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-border hover:bg-accent/5 transition-colors ${
                      idx === filteredAlbums.length - 1 ? 'border-0' : ''
                    }`}
                  >
                    ...
                    <td className="px-6 py-4 text-accent font-bold">{(album.totalStreams / 1000).toFixed(1)}K</td>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {Object.entries(statusConfig).map(([key, config]) => {
          const count = albumsList.filter(a => a.status === key).length;
          return (
